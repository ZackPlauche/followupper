<template>
  <div v-if="contact"
    :class="isModal ? 'fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-0 sm:p-4' : 'min-h-screen'">
    <div
      :class="isModal ? 'bg-slate-800/90 backdrop-blur-sm rounded-none sm:rounded-2xl shadow-2xl border-0 sm:border border-emerald-500/20 w-full h-full sm:h-auto sm:max-w-6xl sm:max-h-[90vh] flex flex-col' : 'bg-slate-800/90 backdrop-blur-sm flex flex-col min-h-screen'">
        ...
        <div class="flex items-start sm:items-center justify-between gap-4">
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap items-center gap-2 sm:gap-3 mt-2">
              ...
              <a v-if="contact.codementor_username" :href="`https://www.codementor.io/@${contact.codementor_username}`"
                target="_blank"
              ...
              <a v-if="contact.email" :href="`https://mail.google.com/mail/?view=cm&to=${contact.email}`"
                target="_blank"
                class="text-blue-400 hover:text-blue-300 text-xs flex items-center gap-1 transition-colors">
      ...
      <div class="flex-1 overflow-y-auto">
        <div class="flex flex-col lg:flex-row h-full">
          ...
          <div class="flex-1 flex flex-col p-4 sm:p-6 border-b lg:border-b-0 lg:border-r border-slate-600/30 space-y-4 sm:space-y-6">
            <!-- Contact Information -->
              ...
              <form @submit.prevent="handleUpdateContact" class="space-y-3">
                ...
                <div>
                  ...
                  <div class="flex items-center bg-slate-600/50 rounded-lg p-1 border border-slate-500/30 w-fit">
                    <button @click="editingContact.gender = ''"
                ...
                <PlatformMultiSelect v-model="editingContact.platform_preference"
                  :available-platforms="preferenceAvailablePlatforms" label="Platform Preference"
            ...
            <div class="bg-slate-700/30 rounded-xl p-4 border border-slate-600/30">
              ...
              <div v-else class="space-y-3">
                <div v-for="(note, index) in notes" :key="index"
                  ...
                  <div v-if="editingNoteIndex !== index" @click="startEditingNote(index)"
                  ...
                  <textarea v-else v-model="editingNoteText" @blur="saveNote(index)"
                    @keydown.ctrl.enter="saveNote(index)" @keydown.esc="cancelEditingNote" rows="3"
            ...
            <div class="bg-slate-700/30 rounded-xl p-4 border border-slate-600/30">
              <div class="flex items-center justify-between mb-4">
                <h4 class="text-lg font-light text-slate-100">{{ isChainMode ? 'Send Message Chain' : 'Send Message' }}
                </h4>
                <div class="flex items-center bg-slate-600/50 rounded-lg p-1 border border-slate-500/30">
                  <button @click="isChainMode = false"
                  ...
                  <button @click="handleChainModeToggle"
                    :class="isChainMode ? 'bg-slate-500/50 text-slate-100' : 'text-slate-400 hover:text-slate-200'"
              ...
              <div v-if="!isChainMode" class="space-y-3">
                ...
                <!-- Platform Selection -->
                <PlatformMultiSelect v-model="singleMessage.platforms"
                  :available-platforms="singleMessageAvailablePlatforms" label="Platforms"
                ...
                <div v-if="singleMessage.schedule" class="space-y-3">
                  <div class="flex items-end gap-3">
                    ...
                    <div>
                      ...
                      <div class="flex items-center bg-slate-600/50 rounded-lg p-1 border border-slate-500/30 w-fit">
                        <button @click="singleMessage.timezone = 'my'"
                        ...
                        <button @click="singleMessage.timezone = 'user'"
              ...
              <div v-else class="space-y-4">
                ...
                <div>
                  <label class="block text-xs font-light text-slate-300 mb-2">Footer/Signature (applies to all
                    messages)</label>
                ...
                <!-- Platform Selection -->
                <PlatformMultiSelect v-model="chainSettings.platforms" :available-platforms="chainAvailablePlatforms"
                  label="Platforms" label-class="block text-xs font-light text-slate-300 mb-2" />
                ...
                <div class="space-y-3">
                  <div>
                    ...
                    <div class="flex items-center bg-slate-600/50 rounded-lg p-1 border border-slate-500/30 w-fit">
                      <button @click="chainSettings.timingType = 'interval'"
                      ...
                      <button @click="chainSettings.timingType = 'specific'"
                  ...
                  <div v-if="chainSettings.timingType === 'interval'" class="space-y-2">
                    <div class="flex items-center space-x-2">
                      ...
                      <label for="send-first-immediately" class="text-xs text-slate-300 cursor-pointer">Send first
                    ...
                    <div class="space-y-2">
                      <div class="flex items-end gap-2">
                        ...
                        <div>
                          <label class="block text-xs font-light text-slate-300 mb-1">Timezone</label>
                          <div
                            class="flex items-center bg-slate-600/50 rounded-lg p-1 border border-slate-500/30 w-fit">
                            <button @click="chainSettings.timezone = 'my'"
                              :class="chainSettings.timezone === 'my' ? 'bg-slate-500/50 text-slate-100' : 'text-slate-400 hover:text-slate-200'"
                              class="px-2 py-1.5 rounded-md transition-colors text-xs font-light">
                              Mine
                            </button>
                            <button @click="chainSettings.timezone = 'user'"
          ...
          <div class="w-full lg:w-1/3 relative pb-4 sm:pb-6 flex flex-col">
            <div class="flex flex-col p-4 sm:p-6 min-h-0 lg:sticky lg:top-0">
              <div class="flex items-center justify-between mb-4">
              ...
              <div v-if="!showMessageHistory">
                ...
                <div v-else class="space-y-3 overflow-y-auto">
                  <div v-for="(item, index) in allUpcomingMessages" :key="item.id || `msg-${index}`"
                      ...
                      <div class="flex items-start justify-between mb-2">
                        <div class="flex-1">
                          <div v-if="item.type === 'message_sequence'" class="flex items-center gap-2 mb-1">
                            <div class="flex items-center gap-1">
                              ...
                              <span class="text-slate-100 font-medium text-sm">{{ getSequenceTitle(item.sequenceId)
                                }}</span>
                            </div>
                            <span class="text-xs text-slate-400">({{ item.unsentCount }} remaining)</span>
                          </div>
                          <div
                            v-else-if="item.type === 'standalone' && editingMessage && !editingMessage.sequenceId && editingMessage.messageId === item.message?.id"
                            class="space-y-3">
                            <div v-if="item.message?.platforms?.includes('email')">
                              <label class="block text-xs font-light text-slate-300 mb-1">Subject</label>
                              <input v-model="editingMessageData.subject" type="text"
                            ...
                            <div>
                              ...
                              <textarea v-model="editingMessageData.body" rows="4"
                          ...
                          <div v-else class="text-slate-100 font-medium text-sm">{{ item.campaign_name || item.title }}
                          </div>
                          <div
                            v-if="item.type !== 'standalone' || !editingMessage || editingMessage.sequenceId !== null || editingMessage.messageId !== item.message?.id"
                            class="text-xs text-slate-400 mb-2">{{ item.campaign_description || item.description ||
                              (item.type === 'standalone' && item.message ? (item.message.body?.substring(0, 100) +
                                (item.message.body?.length > 100 ? '...' : '')) : 'No description') }}</div>
                          <div class="flex flex-col space-y-2 text-xs">
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                              <span v-if="item.status" :class="getStatusClass(item.status)"
                                class="inline-flex px-2 py-1 rounded-full text-xs w-fit">
                                {{ item.status }}
                              </span>
                              <span v-else
                                class="inline-flex px-2 py-1 rounded-full text-xs w-fit bg-blue-500/20 text-blue-400 border border-blue-500/30">
                                Scheduled
                              </span>
                        ...
                        <div class="flex items-start gap-2">
                          <button v-if="item.type === 'standalone' && item.message && item.message.status === 'pending'"
                            ...
                            class="text-emerald-400 hover:text-emerald-300 transition-colors" title="Send now">
                            <Icon name="lucide:send" class="w-4 h-4" />
                          </button>
                          <button
                            v-if="item.type === 'standalone' && item.message && item.message.status === 'pending' && (!editingMessage || editingMessage.sequenceId !== null || editingMessage.messageId !== item.message.id)"
                            @click.stop="startEditingMessage(null, item.message.id)"
                            class="text-slate-400 hover:text-slate-200 transition-colors" title="Edit message">
                            <Icon name="lucide:pencil" class="w-4 h-4" />
                          </button>
                          <button v-if="!item.type && item.next_message" @click.stop="sendCampaignMessage(item)"
                            class="text-emerald-400 hover:text-emerald-300 transition-colors" title="Send now">
                            <Icon name="lucide:send" class="w-4 h-4" />
                          </button>
                          <button v-if="item.type === 'message_sequence'" @click.stop="cancelSequence(item.sequenceId)"
                          ...
                          <button v-if="item.type === 'message_sequence'" @click.stop="toggleMessageDetail(index)"
                            class="text-slate-400 hover:text-slate-200 transition-colors">
                            <Icon :name="expandedMessageIndex === index ? 'lucide:chevron-up' : 'lucide:chevron-down'"
                              class="w-4 h-4" />
                          </button>
                        </div>
                      </div>
                    ...
                    <div v-if="expandedMessageIndex === index && item.type === 'message_sequence'"
                      class="border-t border-slate-500/30">
                      <div class="p-3 pt-5 flex flex-col items-center">
                        <template v-for="(msg, msgIndex) in getSequenceMessages(item.sequenceId)"
                          :key="msg.id || msgIndex">
                          <!-- Chain connector line -->
                          <div v-if="msgIndex > 0" class="w-[1px] h-6 "
                            :class="msg.status === 'sent' ? 'bg-emerald-500/70' : 'bg-slate-500/30'"></div>
                          <div class="relative w-full bg-slate-700/50 rounded-lg p-3 border "
                            :class="msg.status === 'sent' ? 'border-emerald-500/70' : 'border-transparent'">
                            <div
                              class="absolute top-0 left-0 -translate-x-1/2 -translate-y-1/2 w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium z-10 border-2"
                            ...
                            <div>
                              <div class="flex items-center justify-between mb-2 flex-wrap gap-2">
                                ...
                                <div class="flex items-center gap-2 text-xs">
                                  ...
                                  <button
                                    v-if="msg.status === 'pending' && (!editingMessage || editingMessage.sequenceId !== item.sequenceId || editingMessage.messageId !== msg.id)"
                              ...
                              <!-- Edit Mode -->
                              <div
                                v-if="editingMessage && editingMessage.sequenceId === item.sequenceId && editingMessage.messageId === msg.id"
                                class="space-y-3">
                              ...
                              <!-- View Mode -->
                              <div v-else>
                                <div v-if="msg.subject" class="text-slate-100 text-sm font-medium mb-2">{{ msg.subject
                                  }}</div>
                                <div class="text-slate-300 text-sm whitespace-pre-wrap">{{ msg.body || 'No content' }}
                                </div>
                              </div>
                            </div>
                          </div>
    ...
    <div v-if="showAddNoteForm"
      ...
      <div
        class="bg-slate-800/90 backdrop-blur-sm rounded-2xl shadow-2xl border border-emerald-500/20 p-8 w-full max-w-md">
